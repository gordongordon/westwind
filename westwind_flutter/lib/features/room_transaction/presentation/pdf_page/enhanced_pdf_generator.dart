...
/// Enhanced content configuration for PDF files with multi-column support
class EnhancedPdfContentConfig<T> {
  final String title;
  ...
  /// Column overflow settings
  final bool enableColumnOverflow;
  final int columnCount;
  final double columnSpacing;
  
  /// If true, fills columns vertically (complete one column before starting the next)
  /// If false, distributes rows evenly across columns horizontally
  final bool fillColumnVertically;
  ...
  EnhancedPdfContentConfig({
    ...
    this.enableColumnOverflow = false,
    this.columnCount = 2,
    this.columnSpacing = 10,
    this.fillColumnVertically = true, // Default to vertical filling
  });
}

/// An enhanced version of GenericPdfGenerator with multi-column support
class EnhancedPdfGenerator {
  static const _darkColor = PdfColor(0.2, 0.2, 0.2); // Dark gray/almost black
  static const _lightColor = PdfColor(1, 1, 1); // White
  
  /// Generate PDF for any entity type with multi-column support
  static Future<Uint8List> generatePdf<T>({
    required PdfPageFormat pageFormat,
    required List<T> entities,
    required EnhancedPdfContentConfig<T> config,
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          config.enableColumnOverflow
            ? _buildMultiColumnContent<T>(
                context, 
                entities, 
                config
              )
  ...
  /// Build multi-column content layout
  static pw.Widget _buildMultiColumnContent<E>(
    pw.Context context,
    List<E> entities,
    EnhancedPdfContentConfig<E> config,
  ) {
    // If we're using vertical filling (fill one column completely before moving to next)
    if (config.fillColumnVertically) {
      // For vertical filling, we'll use a simpler approach that doesn't rely on constraints
      // Instead, we'll calculate a fixed number of items per column based on page size
      
      // For a typical A4 page, we can fit approximately 25-30 rows per page
      // Accounting for headers, footers, and other content, use a conservative estimate
      final int rowsPerColumn = 20;
      ...
      for (int i = 0; i < config.columnCount; i++) {
        // If we've processed all entities, break
        if (processedRows >= entities.length) break;
        
        final startIndex = processedRows;
        final endIndex = (startIndex + rowsPerColumn) > entities.length 
            ? entities.length 
            : startIndex + rowsPerColumn;
        
        // Skip if we'd be creating an empty column
        if (startIndex >= endIndex) break;
        
        final columnEntities = entities.sublist(startIndex, endIndex);
        processedRows += columnEntities.length;
        
        columns.add(
          pw.Expanded(
            child: pw.Padding(
              padding: pw.EdgeInsets.only(
                right: i < config.columnCount - 1 ? config.columnSpacing : 0
              ),
              child: _buildContentTable<E>(
                context,
                columnEntities,
                config.tableHeaders,
                config.columnAlignments,
      ...
      // Return a row containing all columns
      return pw.Row(
    ...
    // Original implementation (horizontal distribution)
    // Calculate approximate items per column based on entity count and column count
    final itemsPerColumn = (entities.length / config.columnCount).ceil();
    
    // Split entities into chunks for each column
    final columns = <pw.Widget>[];
    
    for (int i = 0; i < config.columnCount; i++) {
      final startIndex = i * itemsPerColumn;
      
      // Skip if we've run out of entities
      if (startIndex >= entities.length) break;
      
      final endIndex = (startIndex + itemsPerColumn) > entities.length 
          ? entities.length 
          : startIndex + itemsPerColumn;
      
      // Create a slice of entities for this column
      final columnEntities = entities.sublist(startIndex, endIndex);
      
      // Add a table for this column
      columns.add(
        pw.Expanded(
          child: pw.Padding(
            padding: pw.EdgeInsets.only(
      ...
      children: [
        pw.Expanded(
          child: pw.Container(
            ...
            height: notes.length > 100 ? 36 : 18, // More height for longer notes
            child: pw.FittedBox(
              child: pw.Text(
                notes,
                style: pw.TextStyle(
